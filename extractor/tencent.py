...
class Extractor(BaseExtractor):
    ...
    async def entrance(self, webpage_url, session):
        ...
        async with session.get(webpage_url, headers=headers) as response:
            ...
            category = selector.css('.site_channel .channel_nav[class~="current"]::text').extract()
            result['category'] = ','.join(category) if category else None
            result['cover'] = f'http://vpic.video.qq.com/0/{vid}.png'
            video_create_time = selector.css('head meta[itemprop="datePublished"]::attr(content)').extract_first()
            upload_ts, upload_date = self.strptime(video_create_time)
            result['upload_ts'] = upload_ts
            if upload_ts is None:
                result['upload_ts'] = self.strptime(selector.css(".video_info .date::text").extract_first())[0]
            video_duration = selector.css('head meta[itemprop="duration"]::attr(content)').extract_first()
            video_duration = self.cal_duration(video_duration)
            if video_duration is None:
                VIDEO_INFO = selector.css('script[r-notemplate]').re_first('var VIDEO_INFO = ([\s|\S]*)\s</script>')
                try:
                    VIDEO_INFO = json.loads(VIDEO_INFO)  ## having tag inside
            ...
            if result['view_count'] == 'undefined':
                result['view_count'] = None
            gather_results = await asyncio.gather(*[self.extract_comment_count(selector=selector, session=session),
                                                    self.extract_author_info(selector=selector, session=session),
                                                    self.extract_by_vkey(vid=vid, url=webpage_url, session=session)])
            # result = {**result, **{'comment_count': gather_results[0]}}
            # result = {**result, **gather_results[1]}
            # result = {**result, **gather_results[2]}
            results = self.merge_dicts(
                result,
                {'comment_count': gather_results[0]},
                gather_results[1],
                gather_results[2]
            )
            return results
    ...
    @RequestRetry
    async def extract_comment_count(self, selector, session):
        raw = selector.css('script').re_first('var COVER_INFO = (.*?)\svar COLUMN_INFO =')
        COVER_INFO = json.loads(raw)
        commentId = jmespath.search('commentId.comment_id', COVER_INFO)
        headers = {
            'authority': 'video.coral.qq.com',
            'cache-control': 'max-age=0',
    ...
    async def extract_author_info(self, selector, session):
        ...
        async with session.get(user_page_url, headers=headers) as response:
            ...
            selector = Selector(text=text)
            result = dict()
            result['author'] = self.extract_author_name(selector=selector)
            result['author_url'] = user_page_url
            return result
    ...
    async def extract_by_vkey(self, vid, url, session, chance_left=config.RETRY):
        try:
            Host = choice(['vv.video.qq.com', 'tt.video.qq.com', 'flvs.video.qq.com',
                           'tjsa.video.qq.com', 'a10.video.qq.com', 'xyy.video.qq.com', 'vsh.video.qq.com',
                           'vbj.video.qq.com', 'bobo.video.qq.com', 'h5vv.video.qq.com'])
            extract_by_vkey_headers = self.general_headers(user_agent=self.random_ua())
            extract_by_vkey_headers['Host'] = Host
            ...
            try:
                ...
                traceback.print_exc()
                if chance_left != 1:
                    return await self.extract_by_vkey(vid=vid, url=url, session=session, chance_left=chance_left - 1)
                else:
                    return await self.extract_by_api(url=url, session=session)
            else:
                try:
                    ResJson = json.loads(response_getinfo_text[len('QZOutputJson='):-1])
                except ValueError:
    ...
    async def extract_by_api(self, url, session):
        extract_by_api_headers = self.general_headers(user_agent=self.random_ua())
        extract_by_api_headers['Referer'] = 'http://v.ranks.xin/'
        extract_by_api_headers['X-Requested-With'] = 'XMLHttpRequest'
        params = {'url': url}
        api = 'http://v.ranks.xin/video-parse.php'
        async with session.get(api, headers=extract_by_api_headers, params=params) as response:
            response_json = await response.json()
            play_addr = jmespath.search('data[0].url', response_json)
            return {'play_addr': play_addr} if play_addr else {}
