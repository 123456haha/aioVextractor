...
class Breaker(BaseBreaker):
    ...
    async def breakdown(self, webpage_url, page, session):
        clips_list = await asyncio.gather(*[self.retrieve_user_paging_api(webpage_url=webpage_url,
                                                                          page=page,
                                                                          session=session,
                                                                          )])
    ...
    @RequestRetry
    async def retrieve_user_paging_api(self, webpage_url, page, session):
    ...
    async def extract_user_pageing_api(self, ResText, webpage_url):
        try:
            ...
            for article in selector.css("li[data-articleid]"):
                ...
                ele['upload_ts'] = self.format_upload_ts(
                    article.css('.video-hover-con p[class*="fs_12"]::text').extract_first())
